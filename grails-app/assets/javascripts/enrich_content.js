...
{				
	...
	scriptNodes.each( function() {
		...
		// alert( "OK3 - done parsing enhancement graph!");
		...
		for( var ei = 0; ei < entityAnnotations.length; ei++ )
		{
			...
			if( typeof id === "string" )
			{
				// NOP, we're good as we are
			}
			else
			{
				// we got back more than one value, pick the first one for now
				id = id[0];
			}
			
			// alert( "dc:relation for entityAnnotation: " + id );
			// alert( Object.prototype.toString.call(id));
